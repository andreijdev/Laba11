...
void part5(Node*& root, int& N) {
	...
	elapsed_seconds = chrono::duration_cast<chrono::nanoseconds>(end - start).count();
	cout << "Удаление отрицательных элементов за " << 0.0000000001 * elapsed_seconds << " секунды.\n";
	...
	elapsed_seconds = chrono::duration_cast<chrono::nanoseconds>(end - start).count();
	cout << "Изменение нечетных элементов за " << 0.0000000001 * elapsed_seconds << " секунды.\n";
}
...
int main()
{
	...
	while (1) {
		...
		if (choice == 6 and root != nullptr) {
			...
			elapsed_seconds = chrono::duration_cast<chrono::nanoseconds>(end - start).count();
			cout << "Проверка на сбалансированность " << 0.0000000001 * elapsed_seconds << " секунды.\n";
		}
		else if (choice == 6)
			cout << "Сначала создайте бинарное дерево.\n";
...
//void conversion_of_coefficients(Node* root, int N) { //неправильно, но может понадобиться
//	if (root != nullptr and (root->left != nullptr or root->right != nullptr)) {
//		List<Node*>** head = new List<Node*> * [N];
//		for (int i = 0; i < N; i++)
//			head[i] = nullptr;
//		List<Node*>* tail = nullptr, * tail2 = nullptr, * temp = nullptr;
//		head[0] = new List<Node*>;
//		head[0]->info = root;
//		int i = 0;
//		do {//запоминаю все ветки и их расположение
//			tail = head[i];
//			i++;
//			while (tail != nullptr) {
//				if (tail->info->left != nullptr) {
//					temp = new List<Node*>;
//					if (head[i] != nullptr)
//						tail2->next = temp;
//					else
//						head[i] = temp;
//					tail2 = temp;
//					tail2->info = tail->info->left;
//				}
//				if (tail->info->right != nullptr) {
//					temp = new List<Node*>;
//					if (head[i] != nullptr)
//						tail2->next = temp;
//					else
...
//
//		tail2 = head[i];
//		while (tail2 != nullptr) {//обнуление коэффициентов нижнего ряда
//			tail2->info->coefficient = 0;
//			tail2 = tail2->next;
//		}
//
//		for (i; i > 0; i--) {
//			tail = head[i - 1];//узлы сверху
//			tail2 = head[i];//узлы снизу
//			while (tail != nullptr) {
//				if (tail->info->left != nullptr) {
//					tail->info->coefficient -= abs(tail2->info->coefficient) + 1;
//					tail2 = tail2->next;
//				}
//				if (tail->info->right != nullptr) {
//					tail->info->coefficient += abs(tail2->info->coefficient) + 1;
//					tail2 = tail2->next;
//				}
//				else if (tail->info->left == nullptr) {
//					tail->info->coefficient = 0;
//				}
//				tail = tail->next;
//			}
//		}
